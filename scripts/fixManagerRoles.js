// Script to check manager roles and fix the issue (FIXED VERSION)
...
async function checkAndFix() {
    try {
        ...
        // EXCEPT for owners, admins who should keep their higher-level roles
        console.log('\n🔧 Fixing Manager Roles...');
        console.log('='.repeat(80));

        for (const user of managersWithDepts) {
            // If user is already owner or admin, don't downgrade them
            if (user.companyRole === 'owner' || user.companyRole === 'admin') {
                console.log(`⏩ Skipping ${user.username} - already has higher role: ${user.companyRole}`);
                continue;
            }

            // If user is member or guest, upgrade to manager
        ...
        // Also restore theju to owner if it was changed
        const theju = await User.findOne({ email: 'theju@kt.com' });
        if (theju && theju.companyRole !== 'owner') {
            await User.findByIdAndUpdate(theju._id, { companyRole: 'owner' });
            console.log(`✅ Restored Theju to 'owner' role`);
        }
