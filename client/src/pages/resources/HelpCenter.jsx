...
const CategoryCard = ({ cat }) => {
    ...
    const Icon = cat.icon;
    return (
        <div style={{ background: '#111', border: '1px solid var(--border-subtle)' }}>
            <div style={{ padding: '20px 22px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div>
                    <h3 style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)' }}>{cat.title}</h3>
                    <p style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{cat.count} articles</p>
                </div>
            </div>
            <div>
                {cat.faqs.map((faq, i) => (
                    <div key={i} style={{ borderBottom: i < cat.faqs.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                        <button onClick={() => setOpen(open === i ? null : i)}
                            ...
                            {open === i ? <ChevronUp size={13} style={{ color: 'var(--text-muted)', flexShrink: 0 }} /> : <ChevronDown size={13} style={{ color: 'var(--text-muted)', flexShrink: 0 }} />}
                        </button>
                        {open === i && (
                            <div style={{ padding: '0 20px 16px', fontSize: '13px', color: 'var(--text-muted)', lineHeight: '1.8' }}>
                                {faq.a}
                            </div>
                        )}
                    </div>
                ...
                <div style={{ maxWidth: '640px', margin: '0 auto', padding: '0 24px' }}>
                    ...
                    <div style={{ position: 'relative', maxWidth: '480px', margin: '0 auto' }}>
                        ...
                        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search help articles..."
                            style={{ width: '100%', padding: '12px 14px 12px 40px', background: 'var(--bg-input)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--text-primary)', fontSize: '14px', fontFamily: 'Inter, system-ui, sans-serif', outline: 'none', boxSizing: 'border-box', transition: 'border-color 150ms ease' }}
            ...
            <div style={{ maxWidth: '1160px', margin: '0 auto', padding: '56px 24px' }}>
                ...
                <div style={{ padding: '36px', background: '#111', border: '1px solid rgba(255,255,255,0.07)', display: 'flex', gap: '32px', alignItems: 'center', flexWrap: 'wrap', justifyContent: 'space-between' }}>
                    <div>
                        ...
                        <p style={{ fontSize: '13px', color: 'var(--text-muted)', lineHeight: '1.7' }}>Our support team responds to all requests within 1 business day.</p>
                    </div>
