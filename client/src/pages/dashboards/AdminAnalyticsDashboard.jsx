...
const AdminAnalyticsDashboard = () => {
  ...
  const cultureData = analyticsData?.culture || { updatesPosted: 0, priorityUpdates: 0, avgReactions: 0 };
  const securityData = analyticsData?.security || { twoFAEnabled: false, ssoStatus: 'Not Configured', loginAnomalies: 0 };
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {workspaceHealth.length === 0 ? (
                        <tr>
                          <td colSpan="5" className="py-8 text-center text-slate-500">No workspace data available</td>
                        </tr>
                      ) : (
                ...
                <div className="lg:col-span-2 space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm text-center">
                      <div className="text-3xl font-black text-slate-800 mb-1">{cultureData.updatesPosted}</div>
                      <div className="text-xs font-bold text-slate-500 uppercase">Updates Posted</div>
                    </div>
                    <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm text-center">
                      <div className="text-3xl font-black text-pink-600 mb-1">{cultureData.priorityUpdates}</div>
                      <div className="text-xs font-bold text-slate-500 uppercase">Priority Updates</div>
                    </div>
                    <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm text-center">
                      <div className="text-3xl font-black text-slate-800 mb-1">{cultureData.avgReactions?.toFixed(1)}</div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                    <div className="flex justify-between items-center pb-4 border-b border-slate-100">
                      ...
                      <span className={`text-sm font-bold ${securityData.twoFAEnabled ? 'text-green-600' : 'text-slate-400'}`}>
                        {securityData.twoFAEnabled ? 'Enabled' : 'Not Enabled'}
                      </span>
                    </div>
                    <div className="flex justify-between items-center pb-4 border-b border-slate-100">
                      <span className="text-sm font-bold text-slate-600">SSO Status</span>
                      <span className="text-sm font-bold text-slate-400">{securityData.ssoStatus}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-bold text-slate-600">Login Anomalies</span>
                      <span className="text-sm font-bold text-slate-800">{securityData.loginAnomalies}</span>
                    </div>
                  </div>
                </div>
              </div>
