...
export const SocketProvider = ({ children }) => {
    ...
    const [workspaceListeners, setWorkspaceListeners] = useState([]);
    ...
    useEffect(() => {
        ...
        socket.on('new-message', (data) => {
            ...
            messageListeners.forEach(cb => cb('new-message', data));
        });

        socket.on('message-deleted', (data) => {
            console.log('📢 [Global] message-deleted:', data);
            messageListeners.forEach(cb => cb('message-deleted', data));
        });

        socket.on('message-pinned', (data) => {
            console.log('📢 [Global] message-pinned:', data);
            messageListeners.forEach(cb => cb('message-pinned', data));
        });

        socket.on('message-unpinned', (data) => {
            console.log('📢 [Global] message-unpinned:', data);
            messageListeners.forEach(cb => cb('message-unpinned', data));
        });

        socket.on('read-update', (data) => {
            console.log('📢 [Global] read-update:', data);
            messageListeners.forEach(cb => cb('read-update', data));
        });

        socket.on('reaction-added', (data) => {
            console.log('📢 [Global] reaction-added:', data);
            messageListeners.forEach(cb => cb('reaction-added', data));
        });

        socket.on('reaction-removed', (data) => {
            console.log('📢 [Global] reaction-removed:', data);
            messageListeners.forEach(cb => cb('reaction-removed', data));
        });
    ...
    useEffect(() => {
        ...
        socket.on('workspace-joined', (data) => {
            ...
            workspaceListeners.forEach(cb => cb('workspace-joined', data));
        });

        socket.on('workspace-created', (data) => {
            console.log('📢 [Global] workspace-created:', data);
            workspaceListeners.forEach(cb => cb('workspace-created', data));
        });
    ...
    }, [socket, workspaceListeners]);

    // Register/unregister listeners
    const addChannelListener = useCallback((callback) => {
        setChannelListeners(prev => [...prev, callback]);
        return () => setChannelListeners(prev => prev.filter(cb => cb !== callback));
    }, []);

    const addMessageListener = useCallback((callback) => {
        setMessageListeners(prev => [...prev, callback]);
        return () => setMessageListeners(prev => prev.filter(cb => cb !== callback));
    }, []);

    const addWorkspaceListener = useCallback((callback) => {
        setWorkspaceListeners(prev => [...prev, callback]);
        return () => setWorkspaceListeners(prev => prev.filter(cb => cb !== callback));
    }, []);
