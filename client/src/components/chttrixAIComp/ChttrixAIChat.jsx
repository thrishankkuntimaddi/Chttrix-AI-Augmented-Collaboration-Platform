...
const ChttrixAIChat = ({ onClose, isSidebar = false }) => {
  ...
  const Content = () => (
    <div className={`flex flex-col h-full bg-white ${!isSidebar && "shadow-2xl rounded-lg border border-gray-200"}`}>
      {/* Header */}
      <div className={`flex justify-between items-center px-4 py-3 bg-[#0c77f2] text-white font-semibold text-sm ${!isSidebar && "rounded-t-lg"}`}>
        <span>🤖 ChttrixAI </span>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-gray-50">
        {messages.map((msg, i) => (
          ...
          >
            <div
              className={`max-w-xs px-4 py-2 rounded-lg text-sm ${msg.sender === "ai"
