...
const HelpWhatsNewView = ({ onBack }) => (
    <div style={panelStyle}>
        <div style={headerStyle}>
            ...
            <span style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-primary)' }}>What's New</span>
        ...
        <div style={{ padding: '14px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {TIMELINE.map(({ date, color, title, body }, i) => (
                <div key={i} style={{ paddingLeft: '14px', borderLeft: `2px solid ${color}`, position: 'relative' }}>
                    ...
                    <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '3px' }}>{title}</div>
                    <div style={{ fontSize: '11px', color: 'var(--text-muted)', lineHeight: 1.5 }}>{body}</div>
                </div>
            ))}
        </div>
    </div>
