...
const HelpShortcutsView = ({ onBack }) => (
    <div style={panelStyle}>
        <div style={headerStyle}>
            ...
            <span style={{ fontSize: '13px', fontWeight: 700, color: '#e4e4e4' }}>Shortcuts</span>
        ...
        <div style={{ padding: '8px 14px 14px' }}>
            {[
                ...
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: i < arr.length - 1 ? '1px solid rgba(255,255,255,0.06)' : 'none' }}>
                    <span style={{ fontSize: '13px', color: 'rgba(228,228,228,0.7)' }}>{item.label}</span>
                    <kbd style={{ padding: '3px 8px', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', fontSize: '10px', fontFamily: 'monospace', color: 'rgba(228,228,228,0.5)', letterSpacing: '0.04em' }}>{item.keys}</kbd>
                </div>
            ))}
        </div>
    </div>
